...
const MODEL_CHILDREN: SettingsLeaf[] = [
  ...
  },
  {
    key: "vlm",
    href: "/settings/vlm",
    label: { zh: "视觉语言模型", en: "VLM" },
    blurb: {
      zh: "显式的视觉语言模型槽位（T1 回退层），用于处理图片附件。",
      en: "Explicit vision-language model slot (T1 fallback tier) for image attachments.",
    },
    icon: ScanEye,
...
const STORAGE_PATHS: Record<string, string> = {
  ...
  "/settings/vlm": "data/user/settings/model_catalog.json",
