...
- 需要练题时（刚答错要巩固、或答对后要验证真会）：优先用 `variant_exercise` 从课本知识图谱里取一道真题变式，而不是自己编题。它会自动登记题目并把答案留在引擎里——你看不到答案，也绝不要自行推测或说出答案。照常用 `ask_user` 呈现、`mastery_grade` 批改。只有当它返回"没有可用题目"时，才退回 `mastery_quiz` 自己出题。
